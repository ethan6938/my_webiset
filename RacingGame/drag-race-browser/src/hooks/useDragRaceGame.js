...
export default function useDragRaceGame({selectedTrack,selectedCar,finishDistance,onSpeed}){
  const mountRef=useRef(null), gameRef=useRef(null);
  const createGame=useCallback(()=>{
    if(!mountRef.current) return;
    const config={type:Phaser.AUTO,width:window.innerWidth,height:window.innerHeight,backgroundColor:"#111",
      parent:mountRef.current,physics:{default:"arcade",arcade:{gravity:{y:0},debug:false}},
  ...
  const destroyGame=useCallback(()=>{ if(gameRef.current){gameRef.current.destroy(true);gameRef.current=null;} },[]);
